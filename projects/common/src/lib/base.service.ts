...
export class BaseService {
  ...
  public storeCheck(obj: string, ttl: number = 60 * 60 * 1000) {
    Cache.addCache(
      Constants.CACHE_CHECK_LOGIN,
      obj,
      Cache.COOKIE,
      ttl
    );
  ...
  isLogin():boolean{
    if(this.loginCheck)
      ...
      return false;
  /* console.log("token", this.getCheck());
  this.getCheck()
       if(this.getCheck())
      return true;
    else
  ...
  getCheck(){
    let check = Cache.getCache(Constants.CACHE_CHECK_LOGIN, Cache.COOKIE);
    console.log("chhhhh", check)
    return check;
  }
